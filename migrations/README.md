...
| `020_address_family_per_sector.sql` | M4: the shared-address family (migration 010) is computed within the rule set, so a gîte at a farm's address cannot win the farm's family and drop it from every export |
