...
| `m2_s6_pagesjaunes.py` | Pages Jaunes → `pj_listings.csv` | **currently produces nothing** (Cloudflare). Hardened: real-Chrome persistent profile, abort after 3 blocked communes, done-file poisoning guard (never mark a 0-card page done without PJ markup), immediate state save. V5 adds `--attach` (CDP). CLI: `--pilot N --headful` |
