"""
M6-S12 — Quality gate: read the deliverable BACK and refuse to ship on a defect
==============================================================================
...
Usage:
    ...
    python scripts/m6_s12_check.py --departement 03 --version v1 --strict
"""
...
from m6_lib import OUT_DIR, CHECK_DIR, read_csv, is_aggregator, root_domain  # noqa: E402
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Read the éleveurs deliverable back and gate it")
    ...
    path = OUT_DIR / f"eleveurs_{dept}_{args.version}.xlsx"
    if not path.exists():
    ...
    # Row identity (M6): the registry établissement (SIRET), or the provider
    # row appended when the registry could not confirm it (population_source
    # = provider, SIRET empty or unknown). Registry rows must equal the
    # population exactly; every row key must be unique.
    reg = [r for r in rows if str(r.get("population_source", "")) == "registre"]
    prov = [r for r in rows if str(r.get("population_source", "")) == "provider"]
    key = lambda r: str(r["siret"]) or f"X{r.get('siren','')}|{r['raisonSociale']}|{r['codePostal']}"
    keys = Counter(key(r) for r in rows)
    check(len(reg) == len(ops), f"H1 registry rows == population ({len(reg)} vs {len(ops)}; + {len(prov)} provider-only rows)")
    ...
    # H2 guards OUR handling (Excel turning text into 4,47956E+13), not the registry's.
    src_sirets = {o["siret"] for o in ops if o["siret"]}
    bad_si = [r for r in rows if r["siret"] and (not isinstance(r["siret"], str) or len(str(r["siret"])) != 14
              or (str(r.get("population_source", "")) == "registre" and str(r["siret"]) not in src_sirets))]
    ...
    if args.baseline:
        bpath = OUT_DIR / f"eleveurs_{dept}_{args.baseline}.xlsx"
        if bpath.exists():
            b = load_xlsx(bpath)
