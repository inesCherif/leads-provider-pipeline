"""
M5-S8 — Match harvested listings (PJ, OSM, DATAtourisme, Atout, France Travail, provider) to the population
==========================================================================================================
Copy of the proven `m3ag_s8_match.py` (itself from `m2_s7_match.py`). The
...
Rungs, strongest first: siret_exact → geo_name (≤150 m + name ≥ 0.62) →
geo_only (≤40 m, unique) → addr_street → addr_name → addr_recent →
name_commune → name_fuzzy.

What is new for M5:
  * LIVENESS_SOURCES (datatourisme, atout) carry no phone/e-mail by design —
    they are matched for the liveness score, the website and the capacity,
    so the "no contact data" rejection does not apply to them.
  * `addr_recent`: siblings at one address with no name evidence — the M2
    rule Sam accepted (H27): the most recently created business wins when
    it is at least 1 year younger than the other; otherwise rejected. The
    population carries `date_creation`, M3AG's did not.
  * The payload keeps `listing_url`, `date`, `detail` (capacity, classement,
    dt_type…) for m5_s9.
  * Stopwords are the lodging ones (camping, gîte, domaine… repeat across
    every commune).

Usage:
    python scripts/m5_s8_match.py --departement 63
...
from m5_lib import CHECK_DIR, HEB_STOPWORDS   # noqa: E402

SOURCES = [
    ("pj_listings.csv",       "pagesjaunes",   ";"),
    ("osm_listings.csv",      "osm",           ";"),
    ("dt_listings.csv",       "datatourisme",  ";"),
    ("atout_listings.csv",    "atout",         ";"),
    ("ft_offres.csv",         "france_travail", ";"),
    ("provider_tourisme.csv", "provider",      ";"),
]
LIVENESS_SOURCES = {"datatourisme", "atout"}
...
STOPWORDS = {w.lower() for w in HEB_STOPWORDS} | {"d", "l", "ea"}
    ...
    for fname, label, delim in SOURCES:
        ...
        with p.open(encoding="utf-8-sig", newline="") as fh:
            for r in csv.DictReader(fh, delimiter=delim):
                ...
                out.append(r)
                n += 1
        log.info(f"loaded {fname}: source '{label}' ({n} rows)")
    return out
...
def detail_of(L: dict, src: str) -> str:
    ...
    if src == "provider":
        return f"label={L.get('label','')}; dirigeant={L.get('dirigeant','')}"
    if src == "pagesjaunes":
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Match listings to the M5 population")
    ...
    if not ours_path.exists():
        sys.exit(f"{ours_path} not found — run m5_s2_transform.py first.")
    ...
    for L in listings:
        ...
        elif len(s) >= 9 and src in ("provider", "osm"):
    ...
    log.info(f"  phones on matches      {sum(1 for r in rows if r['phone'] or r['mobile']):>6}  "
             f"businesses with a phone from a measured source (osm/pj): "
             f"{len({r['row_id'] for r in rows if (r['phone'] or r['mobile']) and r['source'] in ('osm','pagesjaunes')})}")
