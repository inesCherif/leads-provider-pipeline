"""
M5-S2 — Transform raw registry payloads into the M5 population CSV
==================================================================
Input : exports/hebergement/checkpoints/api_raw_<dept>.jsonl   (m5_s1)
Output: exports/hebergement/checkpoints/operateurs_<dept>.csv  (COMMA-delimited)
...
the sector-neutral M3AG scripts (m3ag_s13 SIRENE, m3ag_s7 Pages Jaunes,
m3ag_s3 search) run on this file unchanged; M5's own columns are APPENDED.
Adapter semantics: raisonSociale = enseigne when the registry has one, else
the legal name (directories print the trade name); gerant = "Prénom NOM"
of the best operator role (m2_s2.pick_dirigeant: gérant > président >
exploitant > DG; auditors never); flag_hors_agri mirrors flag_public so the
inherited sort orders put public operators last; numeroBio stays empty;
...
Filters, in order, all counted and reported:
  ...
  4. activity guard: the établissement's own activite_principale must be in
     {55.20Z, 55.30Z} (falls back to the legal unit's code when absent)

Flags, never drops:
  * flag_public — nature juridique 7xxx or a public-law name (commune, EPCI,
    syndicat, régie): phase 1 ships private operators only (Ines 2026-09-09);
    these rows wait in the CSV for phase 2.
  * procedure_collective — "Liquidation" when a Liquidateur sits among the
    officers while the état still reads 'A' (M2/M1 rule).
  * type_hebergement — camping / gîte / chambres d'hôtes / résidence /
    village vacances / meublé / non typé, from NAF then name words; later
    sources (Atout France, DATAtourisme, OSM) refine it at export.

Usage:
    python scripts/m5_s2_transform.py --departements 03,63
"""
...
from m5_lib import (CHECK_DIR, DEPARTEMENTS, NAF_SCOPE, NAF_LABELS,  # noqa: E402
                    classify_type, is_public)

# m3ag_s2.COLUMNS verbatim, then the M5 columns.
...
M5_COLUMNS = ["siren", "denomination_legale", "enseigne", "type_hebergement",
              ...
              "source_population"]
COLUMNS = ADAPTER_COLUMNS + M5_COLUMNS

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)-7s %(message)s",
                    datefmt="%H:%M:%S")
log = logging.getLogger("m5_s2")
...
def run_dept(dept: str) -> None:
    ...
    if not raw.exists():
        sys.exit(f"[{dept}] {raw} missing — run m5_s1_acquire.py --departements {dept} first.")
    ...
    for siren, unit in units.items():
        ...
        for e in unit.get("matching_etablissements") or []:
            ...
            typ = classify_type(naf, enseigne, legal_name)
            ...
            rows.append({
                ...
                "type_hebergement": typ, "forme_juridique": clean(forme), "nature_juridique": nj,
    ...
    log.info(f"[{dept}]   public (phase 2) {c['flag_public']}   liquidation flagged {c['flag_liquidation']}"
