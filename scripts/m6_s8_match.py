"""
M6-S8 — Match harvested listings (PJ, OSM, BAF, Agence Bio, provider, DB claims) to the éleveurs population
==========================================================================================================
Copy of the proven `m5_s8_match.py` (itself from `m3ag_s8_match.py` /
`m2_s7_match.py`). The non-negotiables are unchanged:
...
What is specific to M6:
  * Sources are read IN PLACE from two trees: M6's own harvests
    (exports/eleveurs/checkpoints) and the M3AG ones
    (exports/agriculteurs/checkpoints — Pages Jaunes dept 63 lists EVERY
    farmer, OSM and bienvenue-à-la-ferme too). Nothing is copied.
  * `db_claims.csv` (m6_s19) is one row per CLAIM with its own source
    (agencebio, pagesjaunes, osm, site/confirme, corrobore(…), piste,
    client_file → provider) and verdict; each claim is matched as a listing
    of ITS source, so its measured rank survives into m6_s9. Website claims
    are kept only when a validator called them `valide`.
  * The dedup key is (business, source, listing, kind): one business can
    legitimately carry a phone claim AND an e-mail claim from one source.
  * Stopwords are the farm ones (GAEC, EARL, ELEVAGE, geography of 03/63).
...
SOURCES = [
    ...
    (CHECK_DIR / "agencebio_listings.csv",     "agencebio",       ";"),
    (CHECK_DIR / "provider_agri.csv",          "provider",        ";"),
    (CHECK_DIR / "db_claims.csv",              None,              ";"),   # source per row
]
LIVENESS_SOURCES: set = set()
# db_claims sources that never become a witness here
...
STOPWORDS = {w.lower() for w in M6_STOPWORDS} | {"d", "l", "ea"}
...
def days(d: str) -> int:
    """YYYY-MM-DD -> ordinal days (0 when unknown)."""
    try:
...
def load_listings() -> list[dict]:
    ...
    for p, label, delim in SOURCES:
        if not p.exists():
            log.info(f"(skip) {p.name} not present in {p.parent.parent.name} — source '{label}' not harvested")
            continue
        n = 0
        with p.open(encoding="utf-8-sig", newline="") as fh:
            for r in csv.DictReader(fh, delimiter=delim):
                ...
                out.append(r)
                n += 1
        log.info(f"loaded {p.parent.parent.name}/{p.name}: source '{label or 'per row'}' ({n} rows)")
    return out
...
def detail_of(L: dict, src: str) -> str:
    ...
    if src == "datatourisme":
        return f"type={L.get('dt_type','')}; maj={L.get('lastupdate','')}; classement={L.get('classements','')[:80]}"
    if src == "atout":
        return (f"type={L.get('atout_type','')}; classement={L.get('classement','')}; date={L.get('date_classement','')}; "
                f"capacite={L.get('capacite','')}; emplacements={L.get('emplacements','')}")
    if src == "osm":
        return f"kind={L.get('kind','')}; operator={L.get('operator','')}; operator_type={L.get('operator_type','')}; capacity={L.get('capacity','')}; stars={L.get('stars','')}"
    if src == "france_travail":
        return f"{L.get('intitule','')} ({L.get('type_contrat','')}); contact={L.get('contact_nom','')}"
    if src == "provider":
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Match listings to the M6 éleveurs population")
    ...
    if not ours_path.exists():
        sys.exit(f"{ours_path} not found — run m6_s2_transform.py first.")
    ...
    for r in ours:
        ...
        # enseigne / legal name / gérant scored separately (trade-vs-legal-name lesson)
    ...
    for L in listings:
        ...
        if not any(payload.values()) and src not in LIVENESS_SOURCES:
                ...
                if named_hits:
                    if len({r["_id"] for _, r in named_hits}) > 1 and \
                       abs(named_hits[0][0] - named_hits[1][0]) < 5:
                        rejects["ambiguous: 2+ businesses match by geo+name"] += 1
                        continue
                    d, r = named_hits[0]
                    ...
                    if len({r["_id"] for _, r in very_near}) == 1:
                        ...
                        rejects["ambiguous: 2+ businesses within 40 m"] += 1
                        continue
            ...
            if l_house and l_street:
                ...
                if len(ids) == 1:
                    ...
                    if len({r["_id"] for r in named}) == 1:
                        ...
                        # the M2/H27 rule: the youngest business at the address
                        # wins when it is clearly younger (successor businesses)
                        ...
                        if dated[0]["_created"] and (len(dated) < 2 or dated[0]["_created"] - dated[1]["_created"] >= RECENT_GAP_DAYS):
                            ...
                            rejects["ambiguous: same address, no name evidence, same age"] += 1
                            continue

        listing_names = [n for n in (name, L.get("dirigeant") or L.get("contact_nom") or "") if n]
        ...
        row = {
            ...
            "listing_id": L.get("osm_id") or L.get("listing_id") or L.get("dt_id") or L.get("atout_id") or "",
            "listing_url": L.get("url") or L.get("uri") or L.get("detail_url") or "",
            "date": L.get("date_creation") or L.get("lastupdate") or L.get("date_classement") or L.get("check_date") or "",
            "detail": detail_of(L, src),
            "verdict": L.get("verdict") or ("valid" if L.get("email_verified") == "1" else ""),
        }
    ...
    log.info(f"  phones on matches      {sum(1 for r in rows if r['phone'] or r['mobile']):>6}  "
             f"businesses with a phone from a measured/declared source: "
             f"{len({r['row_id'] for r in rows if (r['phone'] or r['mobile']) and r['source'].split('(')[0] in ('osm','pagesjaunes','agencebio','bienvenue_ferme','site/confirme','corrobore')})}  "
             f"provider only: {len({r['row_id'] for r in rows if (r['phone'] or r['mobile']) and r['source'] == 'provider'} - {r['row_id'] for r in rows if (r['phone'] or r['mobile']) and r['source'] != 'provider'})}")
