"""
M2-S17 — Quality gate for the V4 boulangeries deliverable
==========================================================
...
Usage:
    python scripts/m2_s17_check_v4.py
    python scripts/m2_s17_check_v4.py --strict     # warnings also fail
"""
...
XLSX_PATH = PROJECT_ROOT / "exports" / "boulangerie" / "boulangerie_13_v4.xlsx"
# Compare against V3, not V2: each version must beat the one it replaces.
V2_PATH   = PROJECT_ROOT / "exports" / "boulangerie" / "boulangerie_13_v3.xlsx"
...
def main() -> None:
    ...
    if V2_PATH.exists():
        ...
        hard(not regress, "H12 no regression vs V3", "; ".join(regress))
        log.info(f"        (V3 -> V4: rows {len(v2)}->{n}, phones {v2_ph}->{n_ph}, "
                 f"emails {v2_em}->{n_em}, sites {v2_web}->{n_web})")
    else:
        warn(False, "H12 no regression vs V3", "boulangerie_13_v3.xlsx not found — "
