"""
M6-S19 — What Supabase already knows about éleveurs of depts 03 / 63, as listing CSVs
====================================================================================
Two read-only SELECTs, one connection each, opened and closed inside this
script; the rest of M6 never touches the database (the pooler-timeout
lesson, three times).

(1) provider_agri.csv — the client's own files. The M4 `livestock` sector IS
    "Eleveurs_verified (liste de toutes les eleveurs avec Siret).xlsx"; with
    the `agriculture` file it holds ~1,376 live éleveurs in 03/63, every one
    with a phone, 92 % with a dirigeant name — but only ~1 in 3 with a SIRET.
    Provider phones agreed 73 % with measured sources (dept-13 bakeries), so
    this is a WITNESS: corroboration, e-mail candidates, names; on its own it
    reaches the Probable tab only (m6_s14). Provider e-mails carry the S9-G
    SMTP verdict (`email_verified`) — a verified one ships as `vérifié`.
    Élevage = NAF 01.4x / 01.50Z, or, when the provider gave no NAF, an
    élevage word in its Activité label. Rows whose primary sector is
    agriculteurs_bio are left out: m6_s20 brings them with richer data.

(2) db_claims.csv — every phone / e-mail / website claim in
    `staging.contact_points` for a 03/63 company that is a bio operator or
    carries NAF 01.x, WITH its source, verdict and is_dialable. Each claim
    keeps its own rank in m6_s8/m6_s9 (a `pagesjaunes` claim is a Pages
    Jaunes witness, never "provider"); e-mails proven `invalid` there are an
    exclusion list.
...
ELEVAGE_LABEL_RE = (r"(elev|bovin|ovin|caprin|porc|volaille|lait|vache|brebis|chevr|poule|"
...
SELECT v.company_id, v.siren, v.siret, v.legal_name, v.trade_name, v.display_name,
 ...
 WHERE v.department_code = ANY(%s)
   ...
   AND ('agriculture' = ANY(v.sectors) OR 'livestock' = ANY(v.sectors) OR v.naf_code LIKE '01.%%')
   AND (v.naf_code LIKE '01.4%%' OR v.naf_code LIKE '01.50%%'
...
SELECT cp.company_id, cp.kind, cp.value_norm, cp.source, cp.verdict, cp.is_dialable, cp.is_surtaxe,
       c.siren, c.legal_name, c.trade_name, sf.sector,
 ...
 WHERE left(lpad(btrim(s.postal_code::text), 5, '0'), 2) = ANY(%s)
   ...
   AND (sf.sector = 'agriculteurs_bio' OR c.naf_code LIKE '01.%%')
"""

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s %(levelname)-7s %(message)s",
                    datefmt="%H:%M:%S")
log = logging.getLogger("m6_s19")
